...
def test_get_frame_quality_metrics_returns_none_for_missing_file(monkeypatch):
    monkeypatch.setattr(frame_quality, "cv2", _FakeCV2())

    assert frame_quality.get_frame_quality_metrics("missing.jpg") is None


def test_get_frame_quality_metrics_returns_values(monkeypatch, tmp_path):
    image = tmp_path / "img.jpg"
    image.write_bytes(b"x")

    fake_cv2 = _FakeCV2(brightness=123.0, contrast=22.0, laplacian_var=345.0)
    monkeypatch.setattr(frame_quality, "cv2", fake_cv2)
    monkeypatch.setattr(frame_quality, "FRAME_QUALITY_RESIZE_WIDTH", 320)

    metrics = frame_quality.get_frame_quality_metrics(str(image))

    assert metrics is not None
    assert metrics["brightness"] == 123.0
    assert metrics["contrast_stddev"] == 22.0
    assert metrics["laplacian_var"] == 345.0
    assert fake_cv2.resize_calls == 1


def test_are_metrics_usable_false_for_none():
    assert frame_quality.are_metrics_usable(None) is False


def test_are_metrics_usable_respects_thresholds(monkeypatch):
    monkeypatch.setattr(frame_quality, "FRAME_QUALITY_MIN_BRIGHTNESS", 25.0)
    monkeypatch.setattr(frame_quality, "FRAME_QUALITY_MAX_BRIGHTNESS", 230.0)
    monkeypatch.setattr(frame_quality, "FRAME_QUALITY_MIN_CONTRAST_STDDEV", 10.0)
    monkeypatch.setattr(frame_quality, "FRAME_QUALITY_MIN_LAPLACIAN_VAR", 80.0)

    assert frame_quality.are_metrics_usable({
        "brightness": 120.0,
        "contrast_stddev": 30.0,
        "laplacian_var": 200.0,
    }) is True
    assert frame_quality.are_metrics_usable({
        "brightness": 5.0,
